...
def main():
    ...
    print("\n[1/5] Генерация датасета и построение графика...")
    df = generate_dataset(data_count=500, random_state=40)

    print(f"Датасет успешно сформирован! Всего клиентов: {len(df)}")
    ...
    print("\n[2/5] Инициализация модели Нечеткой Логики...")
    fuzzy_evaluator = FuzzyRiskEval()
    print("Функции принадлежности и 5 экспертных правил успешно загружены.")

    print("\n[3/5] Обучение Нейронной Сети (MLPClassifier)...")
    ...
    print(f"  • Accuracy  (Точность общая): {metrics['accuracy']:.2f}")
    print(f"  • Precision (Точность класса): {metrics['precision']:.2f}")
    print(f"  • Recall    (Полнота):         {metrics['recall']:.2f}")
    print(f"  • F1-score  (Сбалансированная):{metrics['f1_score']:.2f}")

    print("\n[4/5] Тестирование на новом клиенте...")
    new_client = {
        'Age': 35,
        'Income': 45000,
        'Credits': 2,
